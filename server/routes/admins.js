...
aRoutes.use(bodyParser.urlencoded({ extended: true }));
	...
	if ( req.body.token && req.body.name ) {
		...
		knex( "users" )
			...
			.then(result => {
				...
				knex( "tokens" )
					.select( "tokenstring" )
					.where({ userid: id, type: "verify" })
					.then(result => {
						let token = result[0].hasOwnProperty( "tokenstring" )
							? result[0].tokenstring
							: null;
						console.log( token )
						...
						if ( token === req.body.token ) {
							...
							knex( "tokens" )
								...
								.then(( ) => {
									knex( "tokens" )
										.insert({ userid: id, tokenstring: authToken, type: "auth" })
										.then(result => {
											res
												.status( 200 )
...
aRoutes.post("/gettoken", ( req, res, next ) => {
	...
	knex( "users" )
		.select( )
		.where({ type: "admin", name: req.body.name })
		.then(searchResult => {
			if ( searchResult.length > 0 ) {
				...
				knex( "tokens" )
					...
					.then(thingie => {
						...
						knex( "tokens" )
							.insert({ userid: userID, tokenstring: verification })
			...
			res
				.status( 200 )
				.json({ body: "If user exists, you should get a text message soon.", userID: userID })
		})
})
